...
## [[ch10b-面试追问深度复盘|第 10B 章：面试追问深度复盘]]

**学什么**：
- CE 延迟优化的完整答题框架：延迟公式 → 三条线展开 → 优先级排序 → 兜底策略
- 为什么 Bi-Encoder 二次过滤是错的：不能破坏上游模块的前提假设
- RRF k 值的正确推导方式：先定权重比，再反推 k 的范围
- BM25 vs 向量检索的消融实验数据和权重策略：不是调出来的，是架构选择的结果
- 好的面试答案的共同结构：定义问题空间 → 方案展开 → 风险兜底

**面试关联**：面试官在基础问题之后深挖工程权衡和第一性原理时的追问
