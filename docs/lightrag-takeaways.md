...
status: "已归档 —— 改进方向经实测否决（2026-09-15）"
...
> 当前状态：**概念层已理解；建议的改进方向从未实现，且 2026-09-15 被实测数据否决。**

---

> ## ⚠️ 先读这段：这个文档的落点已经作废（2026-09-15）
>
> 本文建议的是**改进项目的 KG（知识图谱）检索路**。但在换成真实语料
> （`corpora/fastapi-zh`，1445 chunk）后跑完模块消融，结论是这条路**没有增量**：
>
> | 配置 | MRR | Hit@5 | Prec@5 |
> |---|---|---|---|
> | `graph_only`（图路单独） | **0.0958** | 0.1111 | 0.0222 |
> | `rrf_fusion`（双路，无图） | 0.7833 | 0.9444 | 0.5000 |
> | `full_pipeline`（双路 + CE） | **0.7889** | **1.0000** | **0.5444** |
> | `triple_fusion`（三路 + CE） | 0.7889 | 1.0000 | 0.5333 |
>
> - **双路 → 三路 ΔMRR = 0.0000**，Prec@5 反而从 0.5444 **掉到** 0.5333
> - 图检索在 `graph` 类（本该它擅长的图关联型问题）MRR 只有 **0.0833**，
>   同类别 BM25 是 0.7083、Vector 是 0.8571
> - 结论：`ENABLE_KG` 默认关闭的决策**是对的**，本文的"3 个改进方向"
>   （给三元组加描述 / 关键词分层 / local-global 模式）**不值得投入**
>
> **所以：不要再照着本文的末尾那句"面试可用的一句话"去讲。** 见文末「为什么这条路线没有被采用」。
>
> **仍然有效的部分**：LightRAG 的**源码分析**（下面 `operate.py` 四个关键函数）——
> 尤其 `_build_query_context()` 的 **token 预算截断**设计，与本项目在
> "CE 精排候选过多导致单条 12.8 秒"上暴露的问题属同一类，仍是可借鉴的思路。
...
## mcp-rag-hub 可探索的 3 个改进方向（⚠️ 已否决，见文首警示 —— 以下仅作技术笔记）

> 下面三条**不要当成待办**。它们要改进的 KG 路已被实测判定无增量（ΔMRR = 0）。保留原文只为记录
> "当时评估了什么"，以及给将来"如果真要做图检索"留个起点。
...
## 建议的“最小可行升级”（⚠️ 未实施，且已不必实施）

> 下面这套升级路线**从未开工**；在 09-15 的消融结论出来后，**也不建议再做**。保留仅为记录思路。
...
## 为什么这条路线没有被采用（2026-09-15 补）

**原来的说法（已作废，别这么讲）**：

> ~~"我参考 LightRAG 的设计，规划把 KG 检索从单一的实体匹配升级为 local/global 分层检索：
> low-level 关键词定位实体节点，high-level 关键词定位关系边。"~~

问题：这是一个**只有规划、没有实现、更没有验证**的说法。面试官只要追问一句"做了吗？效果如何？"
就答不上来 —— 而且它指的方向**恰好是错的**。

**现在该怎么讲**（同一段经历，讲成"我用数据否掉了它"）：

> "我读过 LightRAG 的源码，评估过把 KG 检索升级成 local/global 分层检索（low-level 关键词定位实体节点、
> high-level 关键词定位关系边）。但**换到真实语料后跑模块消融，实测这条路没有增量**：
> 三路融合相对双路的 **ΔMRR = 0.0000**，Prec@5 反而下降；图检索在它本该擅长的图关联类问题上
> MRR 只有 0.0833（同类别 BM25 0.7083、向量 0.8571）。**所以我把它降级成了 `ENABLE_KG` 开关、默认关闭，
> 而不是继续投入去优化一条没有收益的路。**
>
> 这段经历真正留下的不是那个方案，而是两个判断：**① 图检索在没有实体消歧和高质量关系抽取的前提下，
> 在中文技术文档上大概率是负收益；② 一个方案该不该做，要在真实语料上量出来，而不是靠它在别的项目里有效。**"

**这个讲法比原来强在哪**：它同时展示了三件事 —— 读过开源框架源码、有量化实验能力、
**能主动砍掉自己投入过的方向**。最后一点比"我做了很多功能"更能说明工程判断力。

---

## 相关

- 消融数据：`experiments/ablation_results.json`、`experiments/category_breakdown.json`
- 结论汇总：`journal/A组验收结果-2026-09-15.md`
- KG 路当初的失效诊断：`journal/2026-08-04-kg-ablation-notes.md`
