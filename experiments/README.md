...
| `llm_evaluation_results.json` | **2026-09-15** | `llm_eval.py` | **新** | ✅ **可用**（18 条可答，裁判是 3b，见下） |
...
- **`llm_evaluation_results.json` 已于 2026-09-15 被新语料版本覆盖**（18 条、`corpora/fastapi-zh`、
  裁判同为 3b）。此前那份是 2026-09-12 的**旧语料 3 条**（生成/裁判同为 3b），它曾是"本机能跑通
  LLM 生成"的唯一原始证据 —— 需要时从 git 历史取回：
  `git show d3270c2:experiments/llm_evaluation_results.json`
  （另有一份 `%TEMP%` 副本：`experiments_backup_20260915/`）。
  ⚠️ 引用它的三维分数前务必确认是"谁当裁判" —— 3b 自评无区分度（18 条里 13 条 Faithfulness 都是 0.70）。
