...
- **延迟剖析（2026-09-15 用新语料重测）**：**CE 精排是绝对瓶颈，占 99.8%** ——
  单条 query 全链路 **34.0 秒**，其中 CE **33.97 秒**；BM25 / 向量 / 图检索 / RRF 四项合计仅 **56 ms**。
  ...
  优化方向：RRF 之后先截断再送 CE（预期降约 60%）。详见 [A 组验收结果](journal/A组验收结果-2026-09-15.md) §6
