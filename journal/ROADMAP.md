...
> **可勾选的实时清单见 [`未完成清单.md`](./未完成清单.md)**（什么时候做完、下一项做什么，看那份）。
> 本节保留**优先级判断与理由**；两份不一致时以清单为准。

### P0 — 阻塞面试演示，必须解决

- [x] ~~**Ollama 在新语料上跑通**~~ ✅ **2026-09-15 01:30 完成**
      - 3b 起服务后，在 `corpora/fastapi-zh` 上跑完 18 条可答：**生成率 18/18 = 100%**，
        单条 14.5~43.8 秒（纯 CPU）。
      - ⚠️ Ollama **不自启**，每次都要手动起；正确姿势见 `docs/开发过程中遇到的问题.md`
        第 6、7 条（**脱离会话启动** + **不要覆盖 `OLLAMA_MODELS`**）。
      - ⚠️ 新坑（09-15 实测）：启动脚本里**必须连 `ALL_PROXY` 一起清**，否则 agent 走 ollama SDK（httpx）
        会全线 `connection_error`，而 `llm_eval` 走 requests 却正常 —— 伪装成"Ollama 挂了"。
      - 完整结果与面试口径：`journal/端到端验收结果-2026-09-15.md`
...
- [x] ~~四类端到端验收~~ ✅ **2026-09-15 完成**（三类跑通 + 第四类被意外覆盖）：
      | 验收 | 结果 |
      |---|---|
      | 18 条可答 · 生成率 | **100%（18/18）**；三维分数受"3b 当裁判"限制，仅供参考 |
      | 10 条拒答 · 拒答率 / 误答率 | **拒答率 90%**（near_miss 5/5 全对）/ 误答率 10% |
      | 3 条多证据 · 引用覆盖率 | 跨文档引用率 0%；**关键词覆盖 答案 50% vs 检索证据 83.3% → 瓶颈在生成层** |
      | Ollama 不可用 · 降级路径 | 脚本层已被意外覆盖（服务不可用时正确标记、未误报为拒答）；界面层待演示确认 |

      明细与两条异常的根因分析：`journal/端到端验收结果-2026-09-15.md`
      - U08 唯一"误答"是**状态误判不是幻觉**（正文写了"没找到"，`status` 却填 `answered`）→ validator 的真实边界
      - ME01 是 `invalid_json`：检索 100% 命中两个 golden 来源，但 3b 输出 JSON 非法 → 刻画了 3b 的能力边界
