---
title: "在 Dify 中复现 RAG 管线：从部署到分块调优的一次实战"
...
1. **分块策略对代码文件远比 Markdown 敏感**：通用滑窗会把 Python 代码切成无意义碎片，应走 AST / 函数级分块（已在 mcp-rag-hub 落地）。
2. **召回测试是 RAG 调优的第一工具**：不要先改提示词，先看召回回来的片段是否有语义。
3. **文件级描述是低成本高收益优化**：几十字注释就能给向量化/BM25 提供文件级语义锚点。
4. **Top K = 7 在该知识库上效果与成本平衡较好**（相比默认 5，token 涨 21%，回答更完整）。
5. **免费/默认搜索工具不适合生产测试**：Agent 遇到限流会反复重试，需加最大迭代/重试上限，或换稳定 API。
...
文章写到一半没有停。基于 Dify 里发现的"代码被通用滑窗切散"问题，我给 `mcp-rag-hub` 写了一个 AST 分块方案（详见 [`ast-chunking-plan.md`](../docs/ast-chunking-plan.md)）并让 Cursor 生成实现：
